...
uint8_t uart_recv_char = 0; //在串口接受中使用
uint8_t recv_over_flag = 0; //接受一包完成
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    HAL_UART_Receive_IT(&huart2, (uint8_t *)&uart_recv_char, 1);
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
static uint8_t protocol_send_obj(cJSON * send)
{
    ...
    uart_send_data(PROTOCOL_UART_NUM, pkt_head, 6);
#endif

    //TODO: 对发送的数据进行压缩
    HAL_UART_Transmit(&huart2, cJSON_print_buf, pkt_len, 0xffff);

#if PROTOCOL_PKT_CRC
    uint8_t pkt_end[2];
    pkt_end[0] = g_pkt_head.pkt_json_end_1;
    pkt_end[1] = g_pkt_head.pkt_json_end_2;
    uart_send_data(PROTOCOL_UART_NUM, pkt_end, 2);
#endif
...
uint8_t protocol_send_set_cfg(brd_cfg *board_cfg)
{
    ...
    cJSON_AddNumberToObject(root, "uart_baud", board_cfg->uart_baud);
    cJSON_AddNumberToObject(root, "open_delay", board_cfg->open_delay);
    cJSON_AddNumberToObject(root, "pkt_fix", board_cfg->pkt_fix);

    cJSON_AddNumberToObject(root, "out_feature", board_cfg->out_feature);
    cJSON_AddNumberToObject(root, "auto_out_feature", board_cfg->auto_out_feature);
    cJSON_AddNumberToObject(root, "out_interval_in_ms", board_cfg->out_interval_in_ms);
...
uint8_t g_hex_feature[196 * 4 + 1]; //放到全局变量，防止堆栈爆炸
...
static uint8_t protocol_prase_cal_pic_fea_ret(cJSON *root)
{
    ...
    /* get code */
    tmp = cJSON_GetObjectItem(info, "code");
    if (tmp == NULL)
    {
        protocol_send_result(PROTOCOL_RET_FAIL, PROTOCOL_TYPE_CAL_PIC_FEA_RET, "can not get info.code");
        goto _exit;
    }
    cb_data.code = tmp->valueint;
#if PROTOCOL_DEBUG
...
#endif
    /* get msg */
    tmp = cJSON_GetObjectItem(info, "msg");
    if (tmp == NULL)
    {
        protocol_send_result(PROTOCOL_RET_FAIL, PROTOCOL_TYPE_CAL_PIC_FEA_RET, "can not get info.msg");
        goto _exit;
    }
    cb_data.msg = tmp->valuestring;
#if PROTOCOL_DEBUG
